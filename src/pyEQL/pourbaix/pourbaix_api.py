...
class Pourbaix_api:
    def __init__(self, mpr=None, comp_dict: dict | None = None):
        ref_db_file = files("pyEQL") / "pourbaix" / "mpr_reference_ion_database.json"
        # ref_db_file = files("pyEQL") / "pourbaix" / "mpr_reference_ion_database_mp.json"
    ...
    def generate_solution_objects(self):
        ...
        ## Old approach that guesses oxidation states from element data
        # converted_ion_dict = {
        #     standardize_formula(f"{ion}[{Element(ion).common_oxidation_states[0]:+d}]"): f"{val} {default_units}"
        #     for ion, val in ion_dict.items()
        # }

        ## New and rigorous approach that considers the ion and charge
        converted_ion_dict = {standardize_formula(f"{ion}"): f"{val} {default_units}" for ion, val in ion_dict.items()}

        pH_values = [2, 7, 12]  # pH sampling or do we need only one pH?
        ...
        print(f"Speciated ions after filtering out common species: {speciated_ions}")
    ...
    def modified_get_ion_reference_data_for_chemsys(self, chemsys: str | list, nbs_db: dict | None = None):
        ...
        # check if already present in ion_data
